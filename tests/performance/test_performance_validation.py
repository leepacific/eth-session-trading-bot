    ...
    def setUp(self):
        ...
        self.benchmark_configs = {
            "small": PipelineConfig(
                data_length=1000, global_search_samples=20, local_refinement_steps=10, mc_simulations=100, parallel_workers=2
            ),
            "medium": PipelineConfig(
                data_length=5000, global_search_samples=50, local_refinement_steps=20, mc_simulations=500, parallel_workers=4
            ),
    ...
    def test_scalability_analysis(self):
        ...
        for data_size in data_sizes:
            ...
            performance_results.append(
                {"data_size": data_size, "execution_time": execution_time, "time_per_datapoint": time_per_datapoint}
            )

            print(f"   📊 데이터 크기 {data_size:,}: {execution_time:.1f}초 " f"({time_per_datapoint*1000:.2f}ms/포인트)")
...
class TestMemoryUsageProfiling(unittest.TestCase):
    ...
    def setUp(self):
        """테스트 설정"""
        self.performance_optimizer = PerformanceOptimizer(PerformanceConfig(max_memory_usage_gb=2.0))
    ...
    def test_memory_usage_during_optimization(self):
        ...
        # 최적화 실행
        config = PipelineConfig(data_length=5000, global_search_samples=30, mc_simulations=200, parallel_workers=2)

        pipeline = OptimizationPipeline(config)
        parameter_space = {"target_r": (2.0, 4.0), "stop_atr_mult": (0.05, 0.2)}
    ...
    def test_cache_memory_management(self):
        ...
        print(
            f"   🧹 정리 후: {after_cleanup_stats['cache_entries']}개 항목, " f"{after_cleanup_stats['cache_size_mb']:.1f}MB"
        )

        # 정리 후 크기가 줄어들어야 함
        self.assertLessEqual(after_cleanup_stats["cache_size_mb"], final_cache_stats["cache_size_mb"])
...
class TestHistoricalBacktestComparison(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 알려진 좋은 파라미터 (기준점)
        self.benchmark_parameters = {"target_r": 3.0, "stop_atr_mult": 0.1, "swing_len": 5, "rr_percentile": 0.25}
    ...
    def test_parameter_sensitivity_analysis(self):
        ...
        for param_name, base_value in self.benchmark_parameters.items():
            ...
            sensitivity_results[param_name] = {"sensitivity": sensitivity, "scores": param_scores, "variations": variations}

            print(f"   🎛️ {param_name} 민감도: {sensitivity:.4f}")

        # 가장 민감한 파라미터 식별
        most_sensitive = max(sensitivity_results.keys(), key=lambda k: sensitivity_results[k]["sensitivity"])
    ...
    def test_regime_performance_consistency(self):
        ...
        for regime_name, regime_data in regimes.items():
            ...
            # 레짐별 백테스트
            regime_trades = self._simulate_backtest_on_data(self.benchmark_parameters, regime_data)
            ...
            if regime_trades:
                ...
                print(
                    f"   📊 {regime_name}: 점수={regime_score:.4f}, "
...
class TestRiskManagementValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.monitor = RealtimeMonitor(MonitoringConfig(daily_loss_limit_pct=0.05, max_consecutive_losses=5))
    ...
    def _generate_test_trades(self) -> List[Dict]:
        ...
        for i in range(200):
            ...
            trades.append({"timestamp": datetime.now() - timedelta(days=200 - i), "pnl_pct": pnl_pct, "win": pnl_pct > 0})
    ...
    def test_position_sizing_risk_limits(self):
        ...
        for balance in test_balances:
            recommendation = self.kelly_sizer.get_position_recommendation(balance, self.test_trades, current_dd=0.0)

            # 리스크 비율 계산
            risk_ratio = recommendation["risk_amount"] / balance

            print(f"   💰 잔고 ${balance:,}: 포지션 ${recommendation['position_size']:.2f}, " f"리스크 {risk_ratio*100:.1f}%")
    ...
    def test_drawdown_scaling_effectiveness(self):
        ...
        for dd_level in dd_levels:
            recommendation = self.kelly_sizer.get_position_recommendation(balance, self.test_trades, current_dd=dd_level)
    ...
    def test_daily_loss_limit_enforcement(self):
        ...
        for i in range(20):
            ...
            print(f"   📉 손실 {i+1}: {daily_loss_pct*100:.1f}% " f"(상태: {status['trading_state']})")
