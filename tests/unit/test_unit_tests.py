...
class TestStatisticalValidator(unittest.TestCase):
    ...
    def test_deflated_sortino_calculation(self):
        ...
        deflated_sortino, passed = self.validator.calculate_deflated_sortino(sortino_ratio, n_tests, n_observations)
    ...
    def test_whites_reality_check(self):
        ...
        p_value, passed = self.validator.whites_reality_check(benchmark_returns, strategy_returns)
...
class TestRealtimeMonitor(unittest.TestCase):
    ...
    def setUp(self):
        """테스트 설정"""
        config = MonitoringConfig(daily_loss_limit_pct=0.05, max_consecutive_losses=3)
    ...
    def test_consecutive_loss_tracking(self):
        ...
        for i in range(4):  # 4회 연속 손실 (한도 3회 초과)
            trade = TradeEvent(
                timestamp=datetime.now(), symbol="BTCUSDT", side="long", quantity=0.1, price=50000.0, pnl=-100.0
            )
...
class TestPerformanceOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        """테스트 설정"""
        config = PerformanceConfig(max_memory_usage_gb=1.0, cache_size_mb=64.0)  # 테스트용 축소
