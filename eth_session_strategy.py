...
class ETHSessionStrategy:
    ...
    def detect_sweeps(self, session_levels):
        ...
        for i in range(len(df)):
            ...
            if (row['high'] > levels['asia_high'] and 
                row['close'] < levels['asia_high']):
            ...
            if (row['low'] < levels['asia_low'] and 
                row['close'] > levels['asia_low']):
    ...
    def _is_funding_time(self, row):
        ...
        for funding_hour in self.params['funding_hours']:
            # 펀딩 시간 전후 15분 회피
            if (funding_hour == hour and minute == 0) or \
               (funding_hour - 1 == hour and minute == 45) or \
               (funding_hour == hour and minute == 15):
                return True
        return False
    ...
    def backtest(self):
        ...
        for i, signal in enumerate(self.signals):
            ...
            position_info = self.risk_manager.calculate_optimal_position(
                entry_price, stop_price, atr, trade_type
            )
            position_info = self.risk_manager.validate_position(
                position_info, entry_price, stop_price, trade_type
            )
            ...
            for j in range(entry_idx + 1, min(entry_idx + self.params['time_stop_bars'] + 1, len(df))):
                ...
                if trade_type == 'long':
                    ...
                    if bar['high'] >= target_price:
                        pnl_result = self.risk_manager.calculate_pnl(
                            position_info, entry_price, target_price, trade_type
                        )
                        trade.update({
                            'exit_time': bar['time'],
            ...
            if trade['exit_time'] is None:
                ...
                pnl_result = self.risk_manager.calculate_pnl(
                    position_info, entry_price, exit_price, trade_type
                )
    ...
    def print_performance_report(self):
        ...
        print(f"📅 기간: {perf['start_date'].strftime('%Y-%m-%d')} ~ {perf['end_date'].strftime('%Y-%m-%d')} ({perf['total_days']}일)")
