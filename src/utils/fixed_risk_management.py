...
class FixedRiskManager:
    ...
    def calculate_optimal_leverage(self, trade_setup: TradeSetup, target_liquidation_prob: float = 0.07) -> float:
        ...
        # 거래소 한도 및 실용적 한도 적용
        optimal_leverage = min(optimal_leverage, self.risk_params.exchange_max_leverage, 200.0)  # 실용적 최대값
    ...
    def calculate_position_size(self, equity: float, trade_setup: TradeSetup) -> FixedPositionSize:
        ...
        optimal_leverage = self.calculate_optimal_leverage(trade_setup, self.risk_params.liquidation_prob)

        # 3. 청산가 계산
        liquidation_price = self.calculate_liquidation_price(trade_setup.entry_price, optimal_leverage, trade_setup.direction)
    ...
    def analyze_leverage_efficiency(self, equity_range: list, sample_trade: TradeSetup) -> pd.DataFrame:
        """계좌 크기별 레버리지 효율성 분석"""
...
def demonstrate_fixed_risk_system():
    ...
    sample_trade = TradeSetup(
        entry_price=2500.0, stop_price=2450.0, target_price=2650.0, direction="long", atr=50.0  # 2% 스톱  # 6% 타겟 (3:1 RR)
    )
