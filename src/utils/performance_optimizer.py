...
class ParallelProcessor:
    ...
    def start_pools(self):
        """풀 시작"""
        # 스레드 풀 (I/O 집약적 작업용)
        self.thread_pool = ThreadPoolExecutor(max_workers=min(self.config.max_workers, 32), thread_name_prefix="opt_thread")

        # 프로세스 풀 (CPU 집약적 작업용)
        if self.config.use_process_pool:
            self.process_pool = ProcessPoolExecutor(
                max_workers=min(self.config.max_workers, mp.cpu_count()), mp_context=mp.get_context("spawn")
            )
    ...
    def process_parallel(self, func: Callable, data: List[Any], use_processes: bool = False) -> List[Any]:
        """병렬 처리 실행"""
        if not data:
            return []
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    pipeline_id,
                    metric.timestamp,
                    metric.memory_stats.process_memory_gb,
                    metric.cpu_percent,
...
'''
    ...
    def run_deployment_tests(self) -> Dict:
        ...
        test_results["overall"] = {"passed": all_passed, "message": "All tests passed" if all_passed else "Some tests failed"}
...
class PerformanceOptimizer:
    ...
    def _generate_recommendations(self, avg_memory: float, avg_cpu: float, avg_cache_hit: float) -> List[str]:
        """최적화 권장사항 생성"""
        recommendations = []
...
def main():
    ...
    # 성능 최적화기 초기화
    config = PerformanceConfig(max_memory_usage_gb=2.0, cache_size_mb=256.0, max_workers=2)  # 테스트용 축소
    ...
    for i in range(5):
        ...
        print(f"   수집 {i+1}: 메모리 {metrics.memory_stats.process_memory_gb:.2f}GB, " f"CPU {metrics.cpu_percent:.1f}%")
