...
class PerformanceEvaluator:
    def __init__(self, constraint_config: ConstraintConfig = None, score_config: ScoreConfig = None):
        """성과 평가자 초기화"""
    ...
    def calculate_metrics(self, trades_df: pd.DataFrame, initial_balance: float = 100000) -> PerformanceMetrics:
        """성과 지표 계산"""
        if len(trades_df) == 0:
    ...
    def aggregate_results(self, results_list: List[PerformanceMetrics], method: str = "median") -> PerformanceMetrics:
        """결과 집계 - 메디안 기반"""
        if not results_list:
            return self._empty_metrics()
    ...
    def rank_candidates(
        self, candidates: List[Tuple[Dict, PerformanceMetrics]], use_stability: bool = True
    ) -> List[Tuple[Dict, PerformanceMetrics, float]]:
        """후보들을 점수 기준으로 랭킹"""
        ranked_candidates = []
