...
class FastDataEngine:
    ...
    def load_data(self, file_path: str, symbol: str = "ETHUSDT", 
    ...
    def cache_indicators(self, df: pd.DataFrame, params: Dict) -> Dict[str, np.ndarray]:
        ...
        indicators['displacement'] = self._calculate_displacement_numba(
            open_price, close, high, low, disp_mult
        )
        
        # 일중 변동성 (간소화된 버전)
        indicators['rr_percentile'] = self._calculate_rr_percentile_numba(
            indicators['tr'], params.get('rr_percentile', 0.13)
        )
    ...
    @staticmethod
    @njit
    def _calculate_atr_numba(high: np.ndarray, low: np.ndarray, 
    ...
    @staticmethod
    @njit
    def _calculate_tr_numba(high: np.ndarray, low: np.ndarray, 
    ...
    @staticmethod
    @njit
    def _find_swing_points_numba(high: np.ndarray, low: np.ndarray, 
                               swing_len: int) -> Tuple[np.ndarray, np.ndarray]:
        """Numba JIT 스윙 포인트 찾기"""
        n = len(high)
    ...
    @staticmethod
    @njit
    def _calculate_displacement_numba(open_price: np.ndarray, close: np.ndarray,
                                    high: np.ndarray, low: np.ndarray,
                                    disp_mult: float) -> np.ndarray:
    ...
    def get_cached_slice(self, start: int, end: int, 
    ...
    def parallel_backtest(self, param_sets: List[Dict], 
                         strategy_func, n_jobs: int = None) -> List[Dict]:
        """병렬 백테스트 실행"""
        if n_jobs is None:
    ...
    def _parallel_backtest_joblib(self, param_sets: List[Dict], 
        ...
        results = Parallel(n_jobs=n_jobs, backend='multiprocessing')(
            delayed(strategy_func)(params) for params in param_sets
        )
        return results
