...
class TimeSeriesValidator:
    ...
    def create_purged_kfold_splits(self, data_length: int, 
                                  avg_holding_period: int) -> List[Tuple[np.ndarray, np.ndarray]]:
        """Purged K-Fold 분할 생성"""
        ...
        for fold in range(n_splits):
            ...
            if len(train_idx) > 0 and len(test_idx) > 0:
                ...
                print(f"     Train: {train_idx[0] if len(train_idx) > 0 else 'N/A'}-{train_idx[-1] if len(train_idx) > 0 else 'N/A'}")
    ...
    def validate_no_data_leakage(self, train_idx: np.ndarray, test_idx: np.ndarray,
                                embargo_bars: int) -> bool:
        """데이터 누수 검증"""
        ...
        for train_point in train_idx:
            ...
            if (test_min - embargo_bars <= train_point < test_min) or \
               (test_max < train_point <= test_max + embargo_bars):
                return False
        
        return True
    
    def run_fold_validation(self, params: Dict, train_idx: np.ndarray, 
                           test_idx: np.ndarray, fold_id: int,
                           strategy_func: Callable) -> Optional[FoldResult]:
        """단일 fold 검증 실행"""
        try:
            # 데이터 누수 검증
    ...
    def run_timeseries_validation(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]],
                                 strategy_func: Callable,
                                 data_length: int = 50000) -> List[Tuple[Dict, float, Dict]]:
        """시계열 검증 실행"""
        ...
        for i, (params, original_score, original_metrics) in enumerate(candidates):
            ...
            for fold_id, (train_idx, test_idx) in enumerate(splits):
                fold_result = self.run_fold_validation(
                    params, train_idx, test_idx, fold_id + 1, strategy_func
                )
                
                if fold_result:
                    fold_results.append(fold_result)
                    print(f"   Fold {fold_id+1}: Score {fold_result.score:.4f}, "
    ...
    def print_validation_results(self, results: List[Tuple[Dict, float, Dict]], 
                               title: str = "시계열 검증 결과"):
        """검증 결과 출력"""
        print(f"\n📊 {title}")
        print("=" * 80)
        ...
        for i, (params, cv_score, stats) in enumerate(results):
            ...
            param_str = ", ".join([f"{k}: {params.get(k, 'N/A'):.3f}" if isinstance(params.get(k), float) 
                                 else f"{k}: {params.get(k, 'N/A')}" for k in key_params])
