...
class MonteCarloSimulator:
    ...
    def calculate_acf_half_life(self, returns: np.ndarray, max_lags: int = 50) -> int:
        ...
        # 범위 제한
        half_life = max(self.config.min_block_size, 
                       min(half_life, self.config.max_block_size))
    ...
    def run_single_simulation(self, trades_df: pd.DataFrame, 
                            params: Dict[str, float], 
                            sim_id: int) -> PerformanceMetrics:
        """단일 시뮬레이션 실행"""
        try:
    ...
    def run_monte_carlo(self, trades_df: pd.DataFrame, 
                       params: Dict[str, float]) -> MonteCarloResult:
        """몬테카를로 시뮬레이션 실행"""
        print(f"\n🎲 몬테카를로 시뮬레이션 시작")
    ...
    def _analyze_simulation_results(self, results: List[PerformanceMetrics], 
                                  original: PerformanceMetrics) -> MonteCarloResult:
        """시뮬레이션 결과 분석"""
        # 유효한 결과만 필터링
        ...
        if len(valid_results) == 0:
            return MonteCarloResult(
                percentiles={}, stability_metrics={}, robustness_score=0,
                passed_criteria=False, simulation_count=0, original_metrics=original
            )
    ...
    def _calculate_robustness_score(self, percentiles: Dict[str, float], 
    ...
    def print_monte_carlo_results(self, result: MonteCarloResult, 
                                title: str = "몬테카를로 시뮬레이션 결과"):
        """몬테카를로 결과 출력"""
        print(f"\n🎲 {title}")
        print("=" * 80)
