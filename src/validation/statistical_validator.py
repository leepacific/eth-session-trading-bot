...
class StatisticalValidator:
    def __init__(self, performance_evaluator: PerformanceEvaluator):
        ...
        # 가중치 설정
        self.weight_config = {"mc_weight": 0.6, "wfo_weight": 0.4}  # 몬테카를로 가중치  # 워크포워드 가중치
    ...
    def calculate_deflated_sortino(self, sortino_ratio: float, n_tests: int, n_observations: int) -> Tuple[float, bool]:
    ...
    def whites_reality_check(self, benchmark_returns: np.ndarray, strategy_returns: List[np.ndarray]) -> Tuple[float, bool]:
    ...
    def spa_test(self, benchmark_returns: np.ndarray, strategy_returns: List[np.ndarray]) -> Tuple[float, bool]:
    ...
    def calculate_combined_score(self, wfo_result: WalkForwardResult, mc_result: MonteCarloResult) -> float:
        """가중 결합 점수 계산"""
        # WFO OOS 메디안 점수
        ...
        # 가중 결합
        combined_score = self.weight_config["mc_weight"] * mc_score + self.weight_config["wfo_weight"] * wfo_score

        return combined_score

    def validate_candidates(
        self, candidates: List[Tuple[Dict, WalkForwardResult, MonteCarloResult]]
        ...
        for i, (params, wfo_result, mc_result) in enumerate(candidates):
            ...
            wrc_p_value, wrc_passed = self.whites_reality_check(benchmark_returns, all_strategy_returns)
            ...
            # 3. SPA Test
            spa_p_value, spa_passed = self.spa_test(benchmark_returns, all_strategy_returns)
    ...
    def print_validation_results(self, results: List[ValidationResult], title: str = "통계적 검증 결과"):
        """검증 결과 출력"""
        print(f"\n📊 {title}")
        print("=" * 80)
    ...
    def _print_single_result(self, result: ValidationResult, brief: bool = False):
        ...
        if not brief:
            ...
            param_str = ", ".join(
                [
                    (
                        f"{k}: {result.params.get(k, 'N/A'):.3f}"
                        if isinstance(result.params.get(k), float)
                        else f"{k}: {result.params.get(k, 'N/A')}"
                    )
                    for k in key_params
                ]
            )
