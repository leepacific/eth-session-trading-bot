...
class WalkForwardAnalyzer:
    ...
    def detect_volatility_regimes(self, data: pd.DataFrame, 
    ...
    def create_time_based_slices(self, data_length: int, 
                                time_index: pd.DatetimeIndex) -> List[WalkForwardSlice]:
        """시간 기반 슬라이스 생성"""
    ...
    def optimize_slice_parameters(self, slice_obj: WalkForwardSlice, 
        ...
        try:
            ...
            optimized_candidates = self.local_optimizer.run_local_search(
                strategy_func, 
                initial_candidates[:3],  # 상위 3개만 사용
    ...
    def evaluate_oos_performance(self, slice_obj: WalkForwardSlice,
                                test_data: pd.DataFrame,
                                optimal_params: Dict,
                                strategy_func: Callable) -> Tuple[PerformanceMetrics, float]:
        """OOS 성능 평가"""
        try:
            # OOS 전략 실행 (시뮬레이션)
            ...
            print(f"   OOS 평가: PF {oos_metrics.profit_factor:.2f}, "
    ...
    def run_walkforward_analysis(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]],
                                strategy_func: Callable,
                                data: pd.DataFrame) -> WalkForwardResult:
        """워크포워드 분석 실행"""
        ...
        for i, (params, original_score, original_metrics) in enumerate(candidates):
            ...
            for slice_obj in slices:
                ...
                # OOS 성능 평가
                oos_metrics, oos_score = self.evaluate_oos_performance(
                    slice_obj, test_data, optimal_params, strategy_func
                )
    ...
    def print_walkforward_results(self, result: WalkForwardResult, title: str = "워크포워드 분석 결과"):
        ...
        for regime, stats in result.regime_performance.items():
            if stats['count'] > 0:
                print(f"   {regime}: {stats['count']}개 슬라이스, "
        ...
        for slice_obj in result.slices:
            if slice_obj.oos_metrics:
                print(f"   슬라이스 {slice_obj.slice_id} ({slice_obj.regime}): "
