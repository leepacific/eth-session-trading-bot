...
class GlobalSearchOptimizer:
    ...
    def evaluate_candidate(self, params: Dict, fidelity: str, 
                          strategy_func: Callable) -> Tuple[float, PerformanceMetrics]:
        """후보 평가 (특정 충실도에서)"""
        try:
            # 데이터 슬라이스 크기 결정
    ...
    def apply_screening_filter(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]]) -> List[Tuple[Dict, float, PerformanceMetrics]]:
        """스크리닝 필터 적용"""
        filtered = []
        
        for params, score, metrics in candidates:
            # 기본 필터 조건
            if (metrics.profit_factor >= self.screening_filter['min_profit_factor'] and
                metrics.total_trades >= self.screening_filter['min_trades']):
                filtered.append((params, score, metrics))
        
        print(f"🔍 스크리닝 필터: {len(candidates)} → {len(filtered)} 후보")
        return filtered
    
    def apply_asha_pruning(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]], 
                          stage: int) -> List[Tuple[Dict, float, PerformanceMetrics]]:
        """ASHA 조기중단 적용"""
        if len(candidates) == 0:
            return candidates
    ...
    def run_global_search(self, strategy_func: Callable, 
                         sampling_method: str = 'sobol') -> List[Tuple[Dict, float, PerformanceMetrics]]:
        """전역 탐색 실행"""
    ...
    def print_search_results(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]], 
                           top_n: int = 5):
        """탐색 결과 출력"""
