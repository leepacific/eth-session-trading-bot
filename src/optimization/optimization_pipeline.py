...
class OptimizationPipeline:
    ...
    def _execute_stage(self, stage: PipelineStage, stage_func: Callable, 
                      parameter_space: Dict, retry_count: int = 0) -> StageResult:
        """단계 실행"""
    ...
    def _stage_statistical_validation(self, parameter_space: Dict) -> Dict:
        ...
        for candidate in mc_candidates:
            ...
            combined_score = (
                0.6 * candidate['robustness_score'] + 
            ...
            # 통계적 유의성 체크
            passed_stats = (
                statistical_tests['deflated_sortino'] > 1.0 and
                statistical_tests['reality_check_pvalue'] < 0.05 and
                statistical_tests['spa_test_pvalue'] < 0.05
            )
    ...
    def _stage_position_sizing(self, parameter_space: Dict) -> Dict:
        ...
        for balance in balance_levels:
            recommendation = self.kelly_sizer.get_position_recommendation(
                balance, sample_trades, current_dd=0.0
            )
    ...
    def _stage_finalization(self, parameter_space: Dict) -> Dict:
        ...
        summary = {
            ...
            'success_rate': sum(1 for s in self.current_result.stage_results if s.status == PipelineStatus.COMPLETED) / len(self.current_result.stage_results)
        }
