...
class AutoOptimizer:
    ...
    def calculate_objective_score(self, metrics):
        ...
        # 기본 제약 조건
        if (
            metrics["total_trades"] < constraints["min_trades"]
            or metrics["max_drawdown"] > constraints["max_drawdown"]
            or metrics["win_rate"] < constraints["min_win_rate"]
            or metrics["profit_factor"] < constraints["min_profit_factor"]
        ):
            return -10000

        # 추가 제약 조건 (모든 기준 충족)
        if (
            metrics.get("sortino_ratio", 0) < constraints.get("min_sortino_ratio", 0)
            or metrics.get("sharpe_ratio", 0) < constraints.get("min_sharpe_ratio", 0)
            or metrics.get("rr_ratio", 0) < constraints.get("min_rr_ratio", 0)
            or metrics.get("sqn", 0) < constraints.get("min_sqn", 0)
            or metrics.get("calmar_ratio", 0) < constraints.get("min_calmar_ratio", 0)
            or metrics.get("expectancy", 0) < constraints.get("min_expectancy", 0)
        ):
        ...
        # 보너스 점수 (모든 기준 충족 시)
        all_criteria_met = (
            metrics.get("profit_factor", 0) >= 2.0
            and metrics.get("win_rate", 0) >= 0.45
            and metrics.get("win_rate", 0) <= 0.65
            and metrics.get("rr_ratio", 0) >= 2.0
            and metrics.get("expectancy", 0) >= 0.3
            and metrics.get("sortino_ratio", 0) >= 2.0
            and metrics.get("sharpe_ratio", 0) >= 1.5
            and metrics.get("calmar_ratio", 0) >= 3.0
            and metrics.get("sqn", 0) >= 3.0
    ...
    def run_optimization_stage(self, stage_name, stage_config):
        ...
        def objective_wrapper(trial):
            return self.objective_function(
                trial, stage_config["data_points"], enable_walk_forward=stage_config.get("wf_enabled", False)
            )
...
"""
    ...
    def setup_scheduler(self):
        ...
        scheduler.add_job(
            ...
            trigger=CronTrigger(day_of_week="sun", hour=14, minute=0, timezone=kst),  # 일요일  # 14시  # 0분
