...
class LocalSearchOptimizer:
    ...
    def create_optuna_study(self, initial_candidates: List[Tuple[Dict, float, PerformanceMetrics]] = None) -> optuna.Study:
        ...
        study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name='local_search_optimization'
        )
    ...
    def _enqueue_initial_candidates(self, study: optuna.Study, 
                                  candidates: List[Tuple[Dict, float, PerformanceMetrics]]):
        """초기 후보들을 스터디 큐에 추가"""
    ...
    def define_search_space(self, trial: optuna.Trial, 
                          focus_region: Dict[str, Tuple[float, float]] = None) -> Dict:
        """탐색 공간 정의 (국소 영역에 집중)"""
    ...
    def objective_function(self, trial: optuna.Trial, strategy_func: Callable,
                          focus_region: Dict[str, Tuple[float, float]] = None) -> float:
        """목적 함수 (제약 조건 포함)"""
        try:
            # 파라미터 샘플링
    ...
    def calculate_focus_region(self, candidates: List[Tuple[Dict, float, PerformanceMetrics]], 
                             top_n: int = 5) -> Dict[str, Tuple[float, float]]:
    ...
    def run_local_search(self, strategy_func: Callable,
                        initial_candidates: List[Tuple[Dict, float, PerformanceMetrics]] = None,
                        use_focus_region: bool = True) -> List[Tuple[Dict, float, PerformanceMetrics]]:
        """국소 정밀 탐색 실행"""
        print(f"\n🎯 국소 정밀 탐색 시작")
        ...
        for trial in study.trials:
            if trial.state == optuna.trial.TrialState.COMPLETE:
                ...
                metrics = PerformanceMetrics(
                    ...
                    sharpe_ratio=0, calmar_ratio=0, sqn=0, rr_ratio=0,
                    expectancy=0, mar_ratio=0, total_return=0,
    ...
    def print_optimization_progress(self, study: optuna.Study):
        ...
        if study.best_trial:
            ...
            param_str = ", ".join([f"{k}: {best_params.get(k, 'N/A'):.3f}" if isinstance(best_params.get(k), float) 
                                 else f"{k}: {best_params.get(k, 'N/A')}" for k in key_params])
