...
class FailureRecoverySystem:
    ...
    def report_failure(
        self, failure_type: FailureType, description: str, affected_systems: List[str], error_data: Dict = None
    ) -> str:
        """장애 신고"""
    ...
    def _immediate_response(self, failure_event: FailureEvent):
        ...
        if self.monitor:
            self.monitor._send_alert(
                ...
                {"failure_type": failure_event.failure_type.value, "affected_systems": failure_event.affected_systems},
            )
    ...
    def _reduce_positions(self, failure_event: FailureEvent) -> bool:
        ...
        if self.monitor:
            self.monitor._send_alert(
                AlertLevel.WARNING, f"장애 대응: 포지션 {reduction_ratio*100}% 축소", {"reason": failure_event.description}
            )
    ...
    def _trigger_cost_stress_test(self) -> bool:
        ...
        recommendations = []
        if not passed:
            recommendations.extend(["포지션 크기 축소 권장", "거래 빈도 감소 고려", "비용 효율적 전략으로 전환"])
    ...
    def _check_system_health(self):
        ...
        for system in systems:
            ...
            if system in self.system_health:
                ...
                self.system_health[system] = np.clip(self.system_health[system] + change, 0.0, 1.0)
            else:
    ...
    def enable_safe_mode(self, reason: str):
        ...
        if self.monitor:
            self.monitor._send_alert(AlertLevel.CRITICAL, f"안전 모드 활성화: {reason}")
    ...
    def disable_safe_mode(self):
        ...
        if self.monitor:
            self.monitor._send_alert(AlertLevel.INFO, "안전 모드 비활성화")
            ...
            log_data.append(
                {
                    ...
                    "resolution_duration": (
                        (failure.resolution_time - failure.timestamp).total_seconds() if failure.resolution_time else None
                    ),
                }
            )
...
def main():
    ...
    # 장애 복구 시스템 초기화
    recovery_config = FailureRecoveryConfig(cost_multiplier_stress=2.5, max_retry_attempts=2, auto_reduce_threshold=0.03)
    ...
    # 장애 시나리오들
    scenarios = [
        {"type": FailureType.NETWORK_ERROR, "description": "네트워크 연결 불안정", "systems": ["data_feed"]},
        {"type": FailureType.API_ERROR, "description": "거래소 API 오류", "systems": ["trading_engine", "order_system"]},
        {"type": FailureType.COST_SPIKE, "description": "거래 비용 급증", "systems": ["trading_engine"]},
        {"type": FailureType.SYSTEM_ERROR, "description": "시스템 메모리 부족", "systems": ["trading_engine", "risk_manager"]},
    ]
    ...
    for i, scenario in enumerate(scenarios):
        ...
        failure_id = recovery_system.report_failure(
            scenario["type"], scenario["description"], scenario["systems"], {"test_scenario": i + 1}
        )
