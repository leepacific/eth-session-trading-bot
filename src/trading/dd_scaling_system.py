...
class DDScalingSystem:
    ...
    def apply_scaling(self, original_position: float, 
                     reason: str = "DD 기반 축소") -> ScalingResult:
        """포지션에 스케일링 적용"""
        if self.current_state is None:
    ...
    def check_recovery_signal(self) -> bool:
        ...
        recent_dd_improvement = (
            self.current_state.current_dd < self.config.recovery_threshold
        )
        
        # 최근 며칠간 지속적 개선
        recent_states = self.dd_history[-5:]
        dd_trend_improving = all(
            recent_states[i].current_dd >= recent_states[i+1].current_dd
