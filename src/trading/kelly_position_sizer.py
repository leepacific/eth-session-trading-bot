...
class KellyPositionSizer:
    ...
    def calculate_position_size(self, balance: float, trade_stats: TradeStatistics, 
                              current_dd: float = 0.0) -> PositionInfo:
        """포지션 사이즈 계산"""
        print(f"\n💼 포지션 사이징 계산:")
    ...
    def validate_position(self, position_info: PositionInfo) -> bool:
        ...
        print(f"   최소 주문 금액: {'✅' if min_order_ok else '❌'} ${position_info.adjusted_position_size:.2f} >= ${self.params.min_order_amount}")
    ...
    def analyze_kelly_efficiency(self, balance_range: List[float], 
                                sample_trades: List[Dict]) -> pd.DataFrame:
        """켈리 효율성 분석"""
    ...
    def get_position_recommendation(self, balance: float, trades: List[Dict], 
        ...
        # DD 시스템의 동적 스케일링 적용
        dd_recommendation = self.dd_system.get_dynamic_scaling_recommendation(
            position_info.adjusted_position_size
        )
    ...
    def _generate_recommendation(self, position_info: PositionInfo, 
                               trade_stats: TradeStatistics) -> str:
        """추천 메시지 생성"""
...
def main():
    ...
    for balance, dd_level in test_scenarios:
        ...
        recommendation = kelly_sizer.get_position_recommendation(
            balance, sample_trades, dd_level
        )
